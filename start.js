...
const startServers = async () => {
  ...
  // 仅当 LLM=ollama 时启动 Ollama 服务，避免在未安装或无需时导致进程退出
  ...
  const useOllama = ollamaConfig === "ollama";
  ...
  if (useOllama) {
    try {
      ...
      ollamaProcess.on("error", (err) => {
        console.warn("Ollama process failed to start (optional):", err.message);
      });
    } catch (err) {
      console.warn("Could not start Ollama (optional):", err.message);
    }
  }
