...
- 严格遵循指定的幻灯片数量。
- 表格/图表：当页需展示结构化表格时，在 table_data 中输出 {headers: 表头数组, rows: 数据行二维数组}；当页需展示图表（柱状/折线/饼图）时，在 chart_data 中输出 {type: "bar"|"line"|"pie", categories: 分类数组, series: [{name: 系列名, data: 数值数组}]}。无表格或图表时填 null。
