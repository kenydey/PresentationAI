...
def align_image_to_theme_colors(
    image: Image.Image,
    theme_hex_list: list[str],
    strength: float = 0.3,
) -> Image.Image:
    """
    将图像色彩对齐到模板主色调，使整套演示文稿视觉观感一致。
    theme_hex_list: 主色调 Hex 列表，如 ["#1a1a2e", "#16213e", "#0f3460"]
    strength: 调整强度 0~1，越大越接近主题色
    """
    if not theme_hex_list:
        return image
    try:
        import numpy as np
    except ImportError:
    ...
    hex_str = theme_hex_list[0].lstrip("#")
    r_t = int(hex_str[0:2], 16) / 255.0
    g_t = int(hex_str[2:4], 16) / 255.0
    b_t = int(hex_str[4:6], 16) / 255.0

    img_arr = np.array(image.convert("RGB"))
    if img_arr.ndim != 3:
        return image

    img_flat = img_arr.astype(np.float32) / 255.0
    target = np.array([r_t, g_t, b_t], dtype=np.float32)
    for i in range(3):
        mean_val = np.mean(img_flat[:, :, i])
        delta = target[i] - mean_val
        img_flat[:, :, i] = np.clip(
            img_flat[:, :, i] + delta * strength, 0.0, 1.0
        )
    out_arr = (img_flat * 255).astype(np.uint8)
