...
from pipeline import fetch_slide_assets as pipeline_fetch_slide_assets
from pipeline import allocate_asset_paths as pipeline_allocate_asset_paths
...
class PresentationGenerateStreamHandler:
    ...
    async def get_stream(
        ...
        pipeline_allocate_asset_paths(self.presentation_dir, slide_models, "images")
        pipeline_allocate_asset_paths(self.presentation_dir, slide_models, "icons")
        ...
        await pipeline_fetch_slide_assets(
            slide_models, self.theme, self.presentation_dir
        )

        yield SSEResponse(
            event="response",
            data=json.dumps({"type": "status", "status": "Slide assets fetched"}),
        ).to_string()
