...
export async function GET(request: Request) {
  ...
  // Docker/nginx 使用 localhost:80，本地 uv/pip 使用 localhost:5000
  const baseUrl = process.env.SCHEMA_BASE_URL || "http://localhost";
  const schemaPageUrl = `${baseUrl.replace(/\/$/, "")}/schema?group=${encodeURIComponent(groupName)}`;
